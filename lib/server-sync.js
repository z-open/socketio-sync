 ...
 * ex:
 * var sds = syncDataService.subscribe(
            'plan.tasks.sync',
            scope).setParameters({ planId: task.planId });
 * return sds.getData();
 * 
 ...
 *    return dataAccess.executeQuery(queryObj).then(function (r) {
        sync.notifyChanges('TASK_DATA', ç
        return r;
    });
 * }
 * 
 ...
 * 
 * TODO: How to reconnect a client to the same server? load balancing?
 */
var computeTimestamp = require("./timestamp");
...
var publications = {};
var debug = false;
var maxDisconnectionTimeBeforeDroppingSubscription = 10; //seconds
...
// Only notify when the dataNotification matches up with the precise record format. Options:
// {boolean} [forceNotify=false] Notify even if there was no match with the record.
...
function Subscription(user, publication, params) {
    ...
    var queue = {};
    var initialPushCompleted, timestampField;
    ...
    function emitNoDataAtInit() {
        sub.socket.emit('SYNC_NOW', { name: sub.publication.name, subscriptionId: sub.id, records: [], params: sub.params }, function (response) {
    ...
    function emitQueue(isAllRecords) {
        ...
        if (recordsToProcess.length == 0) {
            logSub(sub, 'No data in the queue to emit');
            return;
        }

        if (!sub.socket) {
            logSub(sub, 'Emit canceled. Subscription no longer bound and pending destruction.');
            return;
        }

        logSub(sub, 'Emitting queue:' + recordsToProcess.length);
        sub.timestamp = getMaxTimestamp(sub.timestamp, recordsToProcess);
        sub.socket.emit('SYNC_NOW', { name: sub.publication.name, subscriptionId: sub.id, records: recordsToProcess, params: sub.params, diff: !isAllRecords }, function (response) {
    ...
    function getTimestamp(record) {
        ...
        return record.revision !== null ? record.revision : record.timestamp;
    }
     ...
     */
    function release() {
        logSub(sub, 'Unsubscribed.');
        // ubound socket to this subscription if not done already (disconnect)
        if (sub.socket) {
            var i = sub.socket.subscriptions.indexOf(sub);
            if (i != -1) {
                sub.socket.subscriptions.splice(i, 1);
            }
        }
