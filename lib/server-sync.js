...
function subscribe(handler, subscriptionId, publicationName, params) {
    ...
    if (subscription && handler.user.id === subscription.userId && handler.user.tenantId === subscription.tenantId) {
    ...
    subscription = new Subscription(handler.user, subscriptionId, publication, params);
 ...
 */
function unsubscribe(tenantId, userId, subscriptionId) {
    var subscription = activeSubscriptions[subscriptionId];
    // prevent any other users except the owner to release an existing subscription
    if (subscription && userId == subscription.userId && tenantId === subscription.tenantId) {
        ...
        setTimeout(function () {
            socketSubscriptions.forEach(function (subscription) {
                ...
                if (!subscription.socket) {
                    ...
                    unsubscribe(subscription.tenantId, subscription.userId, subscription.id);
                }
            });
