...
const CURRENT_SYNC_VERSION = '1.1';
...
function Subscription(user, subscriptionId, publication, params) {
    ...
    // client must provide subscriptionId which is uniq to the client (other clients might use the same.)     
    this.id = subscriptionId; //UUID.generate();
    this.userId = user.id;
